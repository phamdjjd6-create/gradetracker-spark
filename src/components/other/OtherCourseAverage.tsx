...
import { CourseAverageRow, GpaScale, GradingScaleEntry } from "@/types/gpa";
import {
...
interface OtherCourseAverageProps {
...
export const OtherCourseAverage: React.FC<OtherCourseAverageProps> = ({
  rows,
  scale,
  gradingScale,
  onRowsChange,
  ...
  const average10 = calculateCourseAverage(rows);
  const average4 = convertToScale4(average10, gradingScale);
  ...
  const fillSample = () => {
    onRowsChange([
      { id: generateId(), name: "Điểm danh", score: 9, weight: 10 },
      { id: generateId(), name: "Bài tập", score: 8.5, weight: 20 },
      { id: generateId(), name: "Giữa kỳ", score: 7.5, weight: 30 },
      { id: generateId(), name: "Cuối kỳ", score: 8, weight: 40 },
    ]);
  };

  const displayValue = scale === 10 ? average10 : average4;
    ...
    <Card className="card-elevated card-hover">
      <CardHeader className="pb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Nhập điểm từng thành phần (tổng trọng số = 100%)
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="pt-4 border-t border-border">
          <GpaDisplay
            value={isWeightValid ? displayValue : 0}
            scale={scale}
            label="Điểm trung bình môn"
            size="md"
          />
          {scale === 4 && isWeightValid && (
            <p className="text-center text-xs text-muted-foreground mt-2">
              Điểm hệ 10: {average10.toFixed(2)}
            </p>
          )}
        </div>
      </CardContent>
